...
class WebSocketBridgeNode(Node):
    ...
    def __init__(self):
        ...
        # 接続ステータス用のタイムスタンプ
        self._last_imu_msg_time = 0.0
        self._last_slam_msg_time = 0.0
        self._current_yaw_source = 'gyro'
        self._current_pos_source = 'acc'
        ...
        self._control_q: queue.Queue = queue.Queue(maxsize=15)
        ...
        self._set_param_client = self.create_client(SetParameters, 'spresense_imu_node/set_parameters')
        self._calib_client = self.create_client(Trigger, 'spresense_imu_node/calibrate')
    ...
    def _process_control_queue(self):
        while True:
            ...
            if cmd == 'reset_pose':
                ...
                self._call_calib_service()
            elif cmd.startswith('set_param:'):
                self._apply_param_change(cmd)
    ...
    def _handle_ws_msg(self, raw: str):
        try:
            ...
            return
        t = data.get('type', '')
        if t in ('reset_pose', 'full_reset', 'start_logging', 'stop_logging', 'calibrate_imu'):
            try:
            ...
            val = data.get('value', '')
            try:
                self._control_q.put_nowait(f'set_param:{name}:{val}')
            except queue.Full:
                pass
    ...
    def _publish_system_status(self):
        ...
        # 2.0秒以内に受信があったかを判定
        imu_ok = (now - self._last_imu_msg_time) < 2.0
        slam_ok = (now - self._last_slam_msg_time) < 2.0
        
        # キャリブレーションのカウントダウン処理
        if self._calib_seconds_left > 0.0:
            self._calib_seconds_left = max(0.0, self._calib_seconds_left - 1.0)
        ...
        payload = json.dumps({
            ...
            'imu_ok': imu_ok,
            'slam_ok': slam_ok,
            'yaw_source': self._current_yaw_source,
            'pos_source': self._current_pos_source,
            'calib_seconds_left': self._calib_seconds_left
        })
        try:
            self._send_q.put_nowait(payload)
        except queue.Full:
    ...
    def _apply_param_change(self, cmd: str):
        # cmd は "set_param:{name}:{val}" の形式
        parts = cmd.split(':')
        if len(parts) < 3:
            return
        name = parts[1]
        ...
        if val.lower() == 'true':
            ...
            try:
                if '.' in val:
                    ...
                    param.value.double_value = float(val)
                else:
                    param.value.type = ParameterType.PARAMETER_INTEGER
                    param.value.integer_value = int(val)
            except ValueError:
                param.value.type = ParameterType.PARAMETER_STRING
                param.value.string_value = val

        if not self._set_param_client.wait_for_service(timeout_sec=1.0):
            self._enqueue_status(False, 'パラメータ設定サービスが見つかりません: spresense_imu_node')
            return
        ...
        def done_callback(fut):
            try:
                ...
                if res and res.results and res.results[0].successful:
                    if name == 'yaw_source':
                        self._current_yaw_source = val
                    elif name == 'pos_source':
                        self._current_pos_source = val
                    self._enqueue_status(True, f'パラメータ {name} を {val} に設定しました')
                else:
                    reason = res.results[0].reason if res and res.results else '不明な理由'
                    self._enqueue_status(False, f'パラメータ設定失敗 ({name}): {reason}')
            except Exception as e:
                self._enqueue_status(False, f'パラメータ設定エラー: {e}')
                
        future.add_done_callback(done_callback)

    def _call_calib_service(self):
        if not self._calib_client.wait_for_service(timeout_sec=1.0):
            self._enqueue_status(False, 'キャリブレーションサービスが見つかりません: spresense_imu_node')
            return
        ...
        def done_callback(fut):
            try:
                ...
                if res and res.success:
                    self._calib_seconds_left = 10.0
                    self._enqueue_status(True, 'IMUキャリブレーションを開始しました。10秒間ロボットを静止させてください。')
                else:
                    msg = res.message if res else '不明なエラー'
                    self._enqueue_status(False, f'IMUキャリブレーション開始失敗: {msg}')
            except Exception as e:
                self._enqueue_status(False, f'キャリブレーションサービスエラー: {e}')
